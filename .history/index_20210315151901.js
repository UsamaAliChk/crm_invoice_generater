...
var con = mysql.createConnection({
  ...
  try{
      con.connect(function(err) {
        if (err) throw err;
       con.query("select contactId from contact",(err,row)=>{
         console.log(row[0].contactId);
       })
      });
...
app.post("/invoice/", (req, res) => {
  ...
  con.query(`insert into invoice (invoiceName,total,contactId) values (${name},${total},${id})`,(err)=>{
    if(err) throw err;
  })
  con.query('select invoiceId from invoice',(err,row)=>{
    if(err) throw err;
    return row
  })
