...
const handelClick=async(id)=>{
     ...
     const company= await axios
        .get(`http://100.25.147.176:5000/company/${id}`)
        .then(res => {return res.data})
        .catch(err => console.error(err));
      const contacts=await axios
        .get(`http://100.25.147.176:5000/contacts/${id}`)
        .then(res => {return res.data})
        .catch(err => console.error(err));
...
const searchByName=(value)=>{
  ...
  const getCompanies=async()=>{
    ...
    const data= await axios
      .get("http://100.25.147.176:5000/companies")
      .then(res => {console.log(res.data); return res.data})
      .catch(err => console.error(err));
      let invoices=await axios.get("http://100.25.147.176:5000/getNoOfInvoices")
      .then(res=>{return res.data})
